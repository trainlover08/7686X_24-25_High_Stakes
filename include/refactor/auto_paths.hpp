...
void red_right_goal_rush () {
    red_right();
    chassis.moveToPose(-DRIVE_LENGTH / 2 - TILE * 1.5, -TILE * 3 + 4 + DRIVE_WIDTH / 2, BLUE_STATION, 5000, {});
    pros::delay(500);
    doinker_piston.extend();
}
...
void blue_left_goal_rush () {
    blue_left();
    chassis.moveToPose(DRIVE_LENGTH / 2 + TILE * 1.5, -TILE * 3 + 4 + DRIVE_WIDTH / 2, RED_STATION, 5000, {});
    pros::delay(500);
    doinker_piston.extend();
}
...
void pre_match_blue_left_elims () {
    chassis.moveToPose(-TILE * 2, DRIVE_WIDTH / 2 + 2 - TILE * 3, RED_STATION, 5000, {.minSpeed=72, .earlyExitRange=6}, 0);
};